...
//
// Communication with the browser half uses a plain HTTP JSON API instead of
// the Typert Remote surface: the client's `ctx.remote` namespaces are
// compiled into the web bundle at build time, so a runtime-installed plugin
// can never be reached that way. Here the host registers an exact web route
// (`/dsh-plugin-manager-api`) and the browser half calls it with fetch().
//
// Methods:
...
export const inject = ["loader", "fs", "clientModules", "webServer"];
...
const API_PATH = "/dsh-plugin-manager-api";
...
export function apply(ctx) {
  ...
  const listEntries = (patchContent) => {
    const loader = ctx.get("loader");
    const rows = [];
  ...
  const writePatch = async (patchPath, content) => {
    const fs = ctx.get("fs");
  ...
  const api = {
    async list() {
      ...
      const content = await readPatch(paths ? paths.patchPath : "");
      const managed = {};
      const map = parseManaged(content);
      for (const [k, v] of map) if (!k.includes(":")) managed[k] = v;
      return { patchPath: paths ? paths.patchPath : null, entries: listEntries(content), managed };
    },
    async setEnabled(args) {
      ...
      const content = await readPatch(paths.patchPath);
      await writePatch(paths.patchPath, ensureArray(applyToggle(content, rowId, enabled)));
    ...
    async deleteEntry(args) {
      ...
      await writePatch(paths.patchPath, ensureArray(removeRowFromPatch(content, rowId)));
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      const args = payload && payload.args !== undefined ? payload.args : {};
      if (method === "list") return send(200, { ok: true, value: await api.list() });
      if (method === "setEnabled") return send(200, { ok: true, value: await api.setEnabled(args) });
      if (method === "deleteEntry") return send(200, { ok: true, value: await api.deleteEntry(args) });
      return send(400, { ok: false, error: "unknown method: " + String(method) });
    } catch (error) {
      return send(500, { ok: false, error: String((error && error.message) || error) });
    }
  };

  const webServer = ctx.get("webServer");
  if (webServer !== undefined && typeof webServer.register === "function") {
    const dispose = webServer.register({ kind: "exact", path: API_PATH, handler });
    ctx.effect(() => dispose, "plugin-manager api route");
  }
}
