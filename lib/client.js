...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Portal support: react-dom is provided by the module table (the bundled
    // ui-primitives import createPortal from it). Portaling the dropdown to
    // document.body lifts it above every modal (the settings panel is a
    // full-viewport modal that covers the shell.overlay layer). Guarded so a
    // missing react-dom degrades to the overlay fallback instead of crashing.
...
`;
    ...
    function apply(ctx) {
      ...
      // Plain HTTP JSON API (host registers /dsh-plugin-manager-api on the
      // web server). The Typert Remote surface is compiled into the web bundle
      // at build time, so a runtime-installed plugin must use its own route.
      const remoteCall = async (method, args) => {
        let response;
        try {
          response = await fetch("/dsh-plugin-manager-api", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ method, args: args === undefined ? {} : args })
          });
        } catch (error) {
          throw new Error("无法连接插件管理服务:" + String((error && error.message) || error));
        }
        let result;
        try {
          result = await response.json();
        } catch {
          throw new Error("插件管理服务响应异常(HTTP " + response.status + ")");
        }
        if (!result || !result.ok) {
      ...
      const MenuOverlay = () => {
        ...
        const menuEl = h("div", { className: "dsh-pmgr-menu", style: { position: "fixed", top, left, zIndex: 2147483000, pointerEvents: "auto" } }, content);
        // Portal to document.body so the dropdown sits above the settings modal
        // (the shell.overlay layer renders below the modal's stacking context).
        return createPortal ? createPortal(menuEl, document.body) : menuEl;
      };
