...
//     rendered at frame level so it is never clipped by the list scroll
// The host half is reached through the Typert Remote service
// `ctx.remote.pluginManager` (methods list / setEnabled / deleteEntry).
...
`;
    ...
    const inject = ["slots", "remote", "remote.pluginManager", "timer"];
    ...
    function apply(ctx) {
      ...
      const remoteCall = async (method, args) => {
        const result = args === undefined
          ? await ctx.remote.pluginManager[method]()
          : await ctx.remote.pluginManager[method](args);
        if (!result.ok) {
